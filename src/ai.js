...
// ── Generate logo image via OpenRouter + Gemini (with modalities) ────────────
...
export async function generateLogoImage(formData, logoBriefing) {
    ...
    // Call OpenRouter with modalities: ["image", "text"] — this is the key!
    const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: 'user',
                    content: `Generate this logo image. Do NOT describe it, just generate the image directly.\n\n${imagePrompt}`,
                },
            ],
    ...
    // Check the images array (OpenRouter returns base64 data URLs here)
    if (message?.images && message.images.length > 0) {
        return message.images[0]; // data:image/png;base64,...
    }

    // Fallback: check content array for multimodal parts
    ...
    // Fallback: check if string content contains base64
    if (typeof message?.content === 'string') {
        const b64Match = message.content.match(/data:image\/[^;]+;base64,[A-Za-z0-9+/=]+/);
        if (b64Match) return b64Match[0];
    }

    throw new Error('O modelo não devolveu imagem. Verifique se tem créditos no OpenRouter.');
}
